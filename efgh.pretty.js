...
function findChild(b, t) {
  return b.children.find((c) => c.type === t);
}
...
function makeBox(t, p) {
  let b = new Uint8Array(8 + p.length),
...
function concatBytes(arr) {
  let out = new Uint8Array(arr.reduce((s, p) => s + p.length, 0)),
    o = 0;
  arr.forEach((p) => {
    out.set(p, o);
    o += p.length;
...
function checkHasCo64(buffer) {
  ...
  try {
    ...
    let vTrak = moov.children.find(
      (c) =>
        c.type === 'trak' &&
        getBoxType(
          findDescendant(c, ['mdia', 'hdlr']).data,
          findDescendant(c, ['mdia', 'hdlr']).offset + 16
        ) === 'vide'
    );
...
function applySharkPatch(buffer) {
  let d = new Uint8Array(buffer),
    v = new DataView(buffer),
    boxes = parseBoxes(d, v);
  let ftyp = boxes.find((b) => b.type === 'ftyp'),
    moov = boxes.find((b) => b.type === 'moov'),
  ...
  let vStts = findChild(stbl, 'stts'),
    vStsc = findChild(stbl, 'stsc'),
    vStsz = findChild(stbl, 'stsz'),
    vStco = findChild(stbl, 'stco');
  ...
  function parseStsz(box) {
    let sSize = box.view.getUint32(box.offset + 12, false),
      count = box.view.getUint32(box.offset + 16, false);
  ...
  function parseStsc(box) {
    let count = box.view.getUint32(box.offset + 12, false),
      rows = [];
    for (let i = 0; i < count; i++) {
      let off = box.offset + 16 + i * 12;
      rows.push([
        box.view.getUint32(off, false),
        box.view.getUint32(off + 4, false),
        box.view.getUint32(off + 8, false),
      ]);
    }
    return rows;
  }
  function parseStco(box) {
    let count = box.view.getUint32(box.offset + 12, false),
      offsets = [];
    for (let i = 0; i < count; i++)
      offsets.push(box.view.getUint32(box.offset + 16 + i * 4, false));
    return offsets;
  }

  let oSizes = parseStsz(vStsz),
    oRows = parseStsc(vStsc),
    oOffsets = parseStco(vStco);
  ...
  let pStts = new Uint8Array(24),
    vts = new DataView(pStts.buffer);
  ...
  let pStsz = new Uint8Array(12 + (realSampleCount + dynamicFakeCount) * 4),
    vsz = new DataView(pStsz.buffer);
  vsz.setUint32(8, realSampleCount + dynamicFakeCount, false);
  oSizes.forEach((sz, i) => vsz.setUint32(12 + i * 4, sz, false));
  for (let i = 0; i < dynamicFakeCount; i++)
    vsz.setUint32(12 + (realSampleCount + i) * 4, F_SIZE, false);

  let pRows = [...oRows];
  if (!pRows.length || pRows[pRows.length - 1][1] !== 1) pRows.push([oOffsets.length + 1, 1, 1]);
  let pStsc = new Uint8Array(8 + pRows.length * 12),
    vsc = new DataView(pStsc.buffer);
  vsc.setUint32(4, pRows.length, false);
  pRows.forEach((r, i) => {
    vsc.setUint32(8 + i * 12, r[0], false);
    vsc.setUint32(12 + i * 12, r[1], false);
    vsc.setUint32(16 + i * 12, r[2], false);
  });
  ...
  function buildStcoRep(stcoBox, delta, fOff) {
    ...
    let p = new Uint8Array(8 + totalCount * 4),
      boxView = new DataView(p.buffer);
    ...
    if (isVideo && fOff !== null) {
      for (let i = 0; i < dynamicFakeCount; i++)
        boxView.setUint32(8 + (origOffsets.length + i) * 4, fOff, false);
    }
    return makeBox('stco', p);
  }

  let fixed = new Map([
    [mdhd, makeBox('mdhd', pMdhd)],
    [elst, makeBox('elst', pElst)],
    [vStts, makeBox('stts', pStts)],
    [vStsc, makeBox('stsc', pStsc)],
    [vStsz, makeBox('stsz', pStsz)],
  ]);
  ...
  let presBytes = concatBytes(
    boxes
      .filter((b) => !['ftyp', 'moov', 'mdat'].includes(b.type))
      .map((b) => b.data.slice(b.offset, b.end))
  );
  let oMdatStart = mdat.cStart,
    oMdatPl = d.slice(mdat.cStart, mdat.end);
  ...
  return concatBytes([
    d.slice(ftyp.offset, ftyp.end),
    rebuild(moov, repFinal),
    presBytes,
    makeBox('mdat', concatBytes([oMdatPl, F_BYTES])),
  ]);
}
  ...
  if (opt === 'fast') {
    ...
    try {
      ...
      if (!window.isEngineLoaded) {
        throw new Error(
          'ဗီဒီယိုဖိုင် Box အပြည့်အစုံမပါဝင်ပါ သို့မဟုတ် 64-bit ဖြစ်နေသည်။ Engine Load ပြီးမှသာ အသုံးပြုနိုင်ပါမည်။'
        );
      }
